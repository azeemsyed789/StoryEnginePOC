...
export default function BuilderAndUserPanel({ role, onLogout }) {
  ...
  const updatePoseAndItems = (poseType, poseValue) => {
    setPages((prev) => {
      ...
      pageUpdate.items = pageUpdate.items.map((item) => {
        if (poseType === "primary" && item.type === "placeholder") return { ...item, pose: poseValue };
        if (poseType === "secondary" && item.type === "secondary") return { ...item, pose: poseValue };
        return item;
      });
  ...
  const removeItem = (id) => updateActivePage({ items: activePage.items.filter((i) => i.id !== id) });
  ...
  const handleDragEnd = (e) => {
    ...
    const newItems = activePage.items.map((item) => {
      if (item.id === active.id) return { ...item, x: item.x + delta.x, y: item.y + delta.y };
      return item;
    });
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white font-sans overflow-hidden">
      ...
      <div className="w-[450px] bg-gray-800 border-r border-gray-700 flex flex-col z-20 shadow-2xl">
        {isAdmin ? (
          // <div className="p-5 overflow-y-auto space-y-6 flex-1">{/* Admin Panel content here */}</div>
          ...
          // <div className="p-8 flex flex-col items-center justify-center h-full space-y-6">{/* User Panel content here */}</div>
